---
title: "Parte 2 — As Plataformas"
sidebar_position: 2
---

# Passo a passo para fazer plataformas ⬇️ ↔️ ↕️ 

![Exemplo de player passando pelos tipos de plataforma](/img/godot_platforms.gif)
...
![Nós renomeados da cena da plataforma](/img/godot_arvore_nos_platform_renomeados.png)
...
![COnfigurações do nó de colisão](/img/godot_collision_platform.png)
...
![Nós renomeados da cena Plataforma de queda](/img/godot_arvore_nos_fall_platform_respawn.png)
...
![Colisões da plataforma de queda](/img/godot_platform_respawn_collisions.png)
...
- Deixe ativado o botão de `Animation Looping` (aparecerá inicialmente com o símbolo de Looping 🔁, clique novamente até mudar a imagem para um simbolo assim    ↔️); 
- Selecione o nó `sprite` e vá na aba `Inspector/Position`;
- Agora clique na 🗝️ da propriedade `Position`, isso adicionará um novo ponto na linha de animação;
...
![Propriedades para animação](/img/godot_configuracoes_anim_plataforma_queda.png)
...
- Clique em `Connect` e então é só partir para o script! 😉
...
![Árvore de nós da cena plataforma móvel](/img/godot_arvore_nos_move_platform_renomeados.png)
...
Agora que criamos uma cena para a plataforma, vamos instanciá-la 🔗 (Atalho: <kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>A</kbd>)  em nossa cena principal e adicionar um novo nó filho do tipo `AnimationPlayer` a essa plataforma, como exemplifica a imagem a seguir:

![Árvore de nós da cena principal](/img/godot_arvore_cena_move_platform.png)
...
1. **Salvar cada posição clicando na chave ao lado da propriedade manipulada**:
    - Você pode utilizar a chave 🔑 ao lado da propriedade `Position` para cada posição que você desejar salvar na linha do tempo da animação, como fizemos em exemplos anteriores.
...
![Configurações de animação da plataforma de movimento](/img/godot_animation_move_platform.png)
...
3. **Exportar Direção de Movimento**:
    ...
    - Na imagem a seguir, podemos observar o resultado dessa exportação. Nas propriedades da animação, podemos escolher qual animação a plataforma terá sem que seja necesário acessar o código novamente.
        ...
        - ![Variáveis externas](/img/godot_move_platform_export_enum.png)
...
![Árvore de nós plataforma bônus](/img/godot_arvore_nos_bonus.png)
...
1. **float**: animação inicial com `"Autoplay on Load"` e `"Animation Looping"` 🔛 habilitados.
...
![Animação flutuando da plataforma](/img/godot_animation_float_bonus.png)
...
![Animação de alerta da plataforma](/img/godot_animation_alert_bonus.png)

3. **falling**: animação de queda da plataforma e subida. `"Animation Looping"`  🔛 desabilitado;
    - Propriedades utilizadas: `texture`, `position` e `rotation`;
    - Propriedade que será alterada: `position`;
...
![Animação de queda e subida da plataforma](/img/godot_animation_falling_bonus.png)
...
Veja o [jogo completo no GitHub](https://github.com/luanabuscariolo/your-first-2D-Platform-Game-with-Godot-4.2/tree/main/2D_Platformer).    
Veja os [outros capítulos do curso](/godot/parte-0-boas-vindas).
